...
# HoloVitals v1.4.9 One-Line Installer
# Installation: wget https://raw.githubusercontent.com/cloudbyday90/HoloVitals/main/scripts/install-v1.4.9.sh && chmod +x install-v1.4.9.sh && ./install-v1.4.9.sh
...
echo "Bug Fix Release - Cloudflare Tunnel in Dev Mode"
echo "=================================================="
echo ""
...
echo "Development: For testing and visual representation (Stripe optional for UI only)"
...
if [[ "$IS_DEV" == "y" || "$IS_DEV" == "Y" ]]; then
    ...
    echo -e "${YELLOW}⚠${NC} Stripe will be optional (for UI testing only)"
else
...
if [[ "$INSTALL_TYPE" == "development" ]]; then
    echo "Stripe: Optional (UI testing only)"
else
