...
# HoloVitals v1.4.10 One-Line Installer
# Installation: wget https://raw.githubusercontent.com/cloudbyday90/HoloVitals/main/scripts/install-v1.4.10.sh && chmod +x install-v1.4.10.sh && ./install-v1.4.10.sh
...
echo "Bug Fix Release - Stripe Conditional Initialization"
echo "=================================================="
echo ""
...
echo "Development: For testing and visual representation"
echo "  - Stripe is OPTIONAL (payment features will be disabled if not configured)"
echo "  - Application will build and run without Stripe keys"
echo "  - Payment UI will show 'not configured' messages"
echo ""
echo "Production: Full deployment with live Stripe payments"
echo "  - Stripe is REQUIRED for payment processing"
echo "  - All payment features fully functional"
echo ""
...
if [[ "$IS_DEV" == "y" || "$IS_DEV" == "Y" ]]; then
    ...
    echo -e "${YELLOW}⚠${NC} Stripe is optional - payment features will be disabled if not configured"
else
    INSTALL_TYPE="production"
    echo -e "${GREEN}✓${NC} Production mode selected"
    echo -e "${YELLOW}⚠${NC} Stripe is required for live payments"
fi
...
if [[ "$INSTALL_TYPE" == "development" ]]; then
    echo "Stripe: Optional (Payment features disabled without keys)"
    echo "  - Application will build successfully without Stripe"
    echo "  - Payment routes will return 'not configured' errors"
    echo "  - Add Stripe keys later to enable payment features"
else
    echo "Stripe: Required (Live payments)"
fi
...
if [[ "$INSTALL_TYPE" == "development" ]]; then
    # Development configuration (Stripe optional)
    cat > .env.local << EOF
...
EOF
    ...
    # Production configuration (Stripe required)
    cat > .env.local << EOF
# Database Configuration
DATABASE_URL="postgresql://holovitals:${DB_PASSWORD}@localhost:5432/holovitals?schema=public"
...
echo ""
echo -e "${YELLOW}⚠${NC} IMPORTANT: Update the following in .env.local:"
if [[ "$INSTALL_TYPE" == "production" ]]; then
    echo "  - STRIPE_SECRET_KEY (required for live payments)"
    echo "  - NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY (required for live payments)"
    echo "  - STRIPE_WEBHOOK_SECRET (required for live payments)"
fi
echo "  - OPENAI_API_KEY"
...
echo "   Required for all installations:"
echo "   - OPENAI_API_KEY"
