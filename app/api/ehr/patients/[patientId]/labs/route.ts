...
export async function GET(
  ...
  try {
    ...
    const queryParams = {
      ...
      category: searchParams.get('category') || 'laboratory',
      startDate: searchParams.get('startDate') || undefined,
      endDate: searchParams.get('endDate') || undefined,
    };

    const validationResult = labsSchema.safeParse(queryParams);
    ...
    await auditService.log({
      ...
      category: 'DATA_ACCESS',
      outcome: 'FAILURE',
      description: `Failed to retrieve lab results: ${error.message}`,
      metadata: {
        error: error.message,
      },
    });
